...
public class cabAdapterFav extends RecyclerView.Adapter<cabAdapterFav.houseViewHolder> {

    ArrayList<cabItem> houseItems = new ArrayList<>();

    cabAdapterFav(ArrayList<cabItem> houseItems) {
    ...
    public houseViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.cab_item_fav, parent, false);
    ...
    public void onBindViewHolder(@NonNull houseViewHolder holder, int position) {
        cabItem currentItem = houseItems.get(position);

        Picasso.with(currentItem.getContext()).load(currentItem.getImageUrl()).resize(810, 450).into(holder.image);
